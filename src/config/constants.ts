...
export const osmUrl = window.env.OSMCHA_OSM_URL || 'https://www.openstreetmap.org'
export const apiOSM = window.env.OSMCHA_OSM_API || 'https://api.openstreetmap.org/api/0.6'
export const adiffServiceUrl = window.env.OSMCHA_ADIFF_SERVICE_URL || 'https://adiffs.osmcha.org'

export const whosThat = 'https://whosthat.osmz.ru/whosthat.php?action=names&id='

export const nominatimUrl =
  window.env.OSMCHA_NOMINATIM_URL || 'https://nominatim.openstreetmap.org/search.php'

// set a default from date x days before today
export const DEFAULT_FROM_DATE = Number(window.env.OSMCHA_DEFAULT_FROM_DATE) || 2
